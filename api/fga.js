...
export default async function fgaHandler(req, res) {
  console.log("🔥 /api/fga was hit:", req.body);
  ...
  const statType = "fga"; // field‑goal attempts
  ...
  try {
    ...
    console.log(
      "🚀 Final fga insights payload:",
      JSON.stringify(insights, null, 2)
    );

    return res.status(200).json({ player, line, direction, insights });
  } catch (err) {
    console.error("❌ Unhandled error in /api/fga:", err);
