...
export default async function orebHandler(req, res) {
  console.log("🔥 /api/oreb was hit:", req.body);
  ...
  const statType = "oreb"; // offensive rebounds
  ...
  try {
    ...
    console.log(
      "🚀 Final oreb insights payload:",
      JSON.stringify(insights, null, 2)
    );

    return res.status(200).json({ player, line, direction, insights });
  } catch (err) {
    console.error("❌ Unhandled error in /api/oreb:", err);
