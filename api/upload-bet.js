...
const buttonThemes = [
  { bookmaker:'prizepicks', keywords:['More','Less'],   selected:{ r:[0,110],  g:[170,255], b:[0,120]  } },
  { bookmaker:'underdog',   keywords:['Higher','Lower'],selected:{ r:[0,80],   g:[180,255], b:[200,255]} },
  { bookmaker:'betrivers',  keywords:['Over','Under'],  selected:{ r:[180,255],g:[0,90],    b:[0,90]   } },
];
...
/* Detect highlighted button per row */
async function detectHighlighted(words,imgBuf){
  const result={};                               // yCoord → 'over'|'under'
  for(const theme of buttonThemes){
    ...
    for(let i=0;i<len;i++){
      const a=btnA[i], b=btnB[i]; if(!a||!b) continue;
      ...
      if(selA||selB){
        ...
        result[a.box.y]=choice; result[b.box.y]=choice;
      }
    }
  }
  return result;
...
export default async function handler(req,res){
  ...
  form.parse(req,async(err,fields,files)=>{
    ...
    try{
      ...
      /* Word boxes for color detection */
      ...
      const topPart=raw.split('\n').slice(0,Math.ceil(raw.split('\n').length/2));
      const betLines=topPart
        .filter(l=>
          /\d+\.\d/.test(l) ||                        // number line
          /(points|rebounds|assists|pt|pra)/i.test(l) // stat keywords
          || /[A-Z][a-z]+\s[A-Z][a-z]+/.test(l)       // name
        )
        .join('\n')
...
- If none found, return [].
      ...
      if(bets.length===0){
        ...
        if(single){
          ...
          // find matching row for color override
          const row=words.find(w=>new RegExp(`${line}`.replace('.','\\.'),'i').test(w.text));
          const fallbackType=row&&colorMap[row.box.y]?colorMap[row.box.y]:'over'; // default
          bets=[{player,prop,line:parseFloat(line),type:fallbackType}];
        }
      }

      /* Normalize & apply color override */
      const final=bets.map(leg=>{
        const baseType=leg.type?.toLowerCase();
        let type=baseType==='more'||baseType==='higher'||baseType==='over'?'over':
                 baseType==='less'||baseType==='lower'||baseType==='under'?'under':baseType;
