...
export default async function tripleDoubleHandler(req, res) {
  console.log("🔥 Triple-Double request hit with:", req.body);
  ...
  try {
    ...
    const insights = await getTripleDoubleInsights({
      playerId: player_id,
      playerName: player,
      teamId: team_id,
      opponentTeamId,
    ...
    console.log("✅ Triple-Double insights:", JSON.stringify(insights, null, 2));
    return res.status(200).json({ player, insights });

  } catch (err) {
    console.error("❌ Unhandled error in /api/triple_double:", err);
